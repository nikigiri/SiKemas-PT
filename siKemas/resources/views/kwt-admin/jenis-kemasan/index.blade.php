    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ route('kwt-admin.dashboard') }}"
                ...
                <div>
                    <h2 class="text-3xl font-bold text-gray-800">Jenis Kemasan</h2>
                    <p class="text-sm text-gray-500 mt-1">Kelola jenis kemasan KWT kamu</p>
                </div>
            </div>
            <div class="relative w-full md:w-80">
                <input type="text" placeholder="Cari kemasan..."
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400 absolute left-4 top-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35m1.85-5.65a7.5 7.5 0 11-15 0a7.5 7.5 0 0115 0z" />
                </svg>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-10 bg-gradient-to-br from-green-50 via-white to-green-100 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 mb-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-800">Daftar Jenis Kemasan</h3>
                        <p class="text-sm text-gray-500 mt-1">Kemasan khusus KWT kamu</p>
                    </div>
                    <a href="{{ route('kwt-admin.jenis-kemasan.create') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse ($jenisKemasans as $kemasan)
                    <div class="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden hover:-translate-y-1 hover:shadow-2xl transition duration-300">
                        <div class="p-6">
                            <div class="flex items-start justify-between">
                                <div class="w-16 h-16 rounded-2xl bg-gradient-to-r from-orange-400 to-amber-500 flex items-center justify-center shadow-lg overflow-hidden">
                                    @if ($kemasan->ikon_kemasan)
                                        <img src="{{ asset($kemasan->ikon_kemasan) }}" alt="{{ $kemasan->nama_kemasan }}" class="w-10 h-10 object-contain">
                                    @else
                                ...
                                <span class="bg-orange-100 text-orange-700 text-xs px-4 py-2 rounded-2xl font-semibold">Kemasan KWT</span>
                            </div>
                            <div class="mt-5">
                                <h3 class="text-xl font-bold text-gray-800">{{ $kemasan->nama_kemasan }}</h3>
                        ...
                        <div class="p-5 flex items-center gap-3">
                            <a href="{{ route('kwt-admin.jenis-kemasan.edit', $kemasan->id) }}"
                            ...
                            <form method="POST" action="{{ route('kwt-admin.jenis-kemasan.destroy', $kemasan->id) }}"
                ...
                @empty
                    <div class="col-span-full">
                        <div class="bg-white rounded-3xl shadow-lg border border-gray-100 px-6 py-16 text-center">
                            <div class="w-28 h-28 mx-auto rounded-full bg-gray-100 flex items-center justify-center mb-6">
                                <svg class="w-14 h-14 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                                </svg>
                            </div>
                            <h3 class="text-2xl font-bold text-gray-700">Belum Ada Jenis Kemasan</h3>
                            <p class="text-gray-400 mt-2">Tambah kemasan khusus KWT kamu</p>
                        </div>
                    </div>
                @endforelse
            </div>
