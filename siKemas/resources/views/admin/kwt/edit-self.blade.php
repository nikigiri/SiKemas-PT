<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-3xl font-bold text-gray-800">Edit Info KWT</h2>
                <p class="text-sm text-gray-500 mt-1">Perbarui data Kelompok Wanita Tani</p>
            </div>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-green-500 to-emerald-600 px-8 py-6 flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-white">Edit Data KWT</h3>
                        <p class="text-green-100 text-sm mt-0.5">Ubah informasi kelompok wanita tani di bawah ini</p>
                    </div>
                </div>

                <form method="POST" action="{{ route('kwt-admin.kwt.update.self') }}">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div>
                            <label for="nama_kwt" class="block text-sm font-semibold text-gray-700 mb-2">
                                Nama KWT <span class="text-red-500 ml-0.5">*</span>
                            </label>
                            <div class="relative">
                                ...
                                <x-text-input id="nama_kwt"
                                    ...
                                    type="text" name="nama_kwt" :value="old('nama_kwt', $kwt->nama_kwt)"
                                    required autofocus placeholder="Contoh: KWT Melati, KWT Seruni..." />
                            </div>
                            <x-input-error :messages="$errors->get('nama_kwt')" class="mt-2" />
                        </div>

                        <div>
                            <label for="no_kwt" class="block text-sm font-semibold text-gray-700 mb-2">No KWT</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                <x-text-input id="no_kwt"
                                    class="block w-full pl-11 pr-4 py-3 border border-gray-200 rounded-2xl shadow-sm focus:ring-2 focus:ring-green-400 focus:border-transparent transition"
                                    type="text" name="no_kwt" :value="old('no_kwt', $kwt->no_kwt)"
                                    placeholder="Contoh: 001/KWT/2024" />
                            </div>
                        </div>

                        <div>
                            <label for="desa" class="block text-sm font-semibold text-gray-700 mb-2">Desa</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                <x-text-input id="desa"
                                    class="block w-full pl-11 pr-4 py-3 border border-gray-200 rounded-2xl shadow-sm focus:ring-2 focus:ring-green-400 focus:border-transparent transition"
                                    type="text" name="desa" :value="old('desa', $kwt->desa)"
                                    placeholder="Nama desa..." />
                            </div>
                        </div>

                        <div>
                            <label for="alamat_kwt" class="block text-sm font-semibold text-gray-700 mb-2">Alamat</label>
                            <textarea id="alamat_kwt" name="alamat_kwt" rows="4"
                                placeholder="Tulis alamat lengkap KWT..."
                                class="block w-full px-4 py-3 border border-gray-200 rounded-2xl shadow-sm focus:ring-2 focus:ring-green-400 focus:border-transparent transition resize-none text-sm text-gray-700 placeholder-gray-400">{{ old('alamat_kwt', $kwt->alamat_kwt) }}</textarea>
